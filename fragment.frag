...
vec4 map(vec3 p){
    ...
    float t1 = .1*sin(utime*.3);
    float t2 = .1*sin(utime*.26);
    float t3 = .1*sin(utime*.22);
...
void main() {
    ...
    FragColor = vec4(uv, 0., 1.); //vec4(1.5*log(1.+ col), 1.0);
}
...
void main(){
    ...
    FragColor = vec4(mapping(p,int(floor(mod(utime,4.)))),1.);
}*/
