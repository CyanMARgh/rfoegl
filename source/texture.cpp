...
Texture _load_texture(const std::string& path, aiTextureType type) {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, (type == aiTextureType_DIFFUSE || type == aiTextureType_SPECULAR) ? GL_SRGB : GL_RGB, texture.width, texture.height, 0, GL_RGB, GL_UNSIGNED_BYTE, image_data);
...
Texture load_texture(const std::string& path, aiTextureType type) {
	if(auto F = loaded_textures.find(path); F != loaded_textures.end()) {
		...
		Texture texture = _load_texture(path, type);
